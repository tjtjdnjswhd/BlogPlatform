...
namespace BlogPlatform.Api.Tests.Controllers
{
    public class PostControllerTests
    {
        ...
        {
            // Act
            IActionResult result = await _postController.RestoreAsync(5, 1, CancellationToken.None);

            // Assert
            Utils.VerifyNotFoundResult(result);
            _setUp.SoftDeleteServiceMock.Verify(s => s.ResetSoftDeleteAsync(It.IsAny<Post>(), It.IsAny<bool>()), Times.Never);
        }
        ...
        {
            // Arrange
            List<Post> posts = [new Post("title0", "content0", 1) { Id = 1, SoftDeleteLevel = 1, SoftDeletedAt = DateTimeOffset.UtcNow }];
            _postDbSetMock = _setUp.DbContextMock.SetDbSet(d => d.Posts, posts);

            // Act
            IActionResult result = await _postController.RestoreAsync(1, 2, CancellationToken.None);
        ...
        [Fact]
        public async Task Restore_BadRequest()
        {
            // Act
            IActionResult result = await _postController.RestoreAsync(1, 1, CancellationToken.None);
        ...
        [Fact]
        public async Task Restore_Expired()
        {
            // Arrange
            List<Post> posts = [new Post("title0", "content0", 1) { Id = 1, SoftDeleteLevel = 1, SoftDeletedAt = DateTimeOffset.UtcNow.AddDays(-2) }];
            _postDbSetMock = _setUp.DbContextMock.SetDbSet(d => d.Posts, posts);
        ...
        public async Task Restore_NoContent()
        {
            ...
            // Assert
            Utils.VerifyNoContentResult(result);
            _setUp.SoftDeleteServiceMock.Verify(s => s.ResetSoftDeleteAsync(It.IsAny<Post>(), true), Times.Once);
        }
    }
}
