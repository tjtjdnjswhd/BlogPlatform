...
namespace BlogPlatform.Api.Tests.Controllers
{
    ...
    public class BlogControllerTests
    {
        ...
        {
            // Act
            IActionResult result = await _blogController.RestoreAsync(5, 1, CancellationToken.None);
        ...
        [Fact]
        public async Task Restore_Forbidden()
        {
            // Arrange
            List<Blog> blogs =
            [
                new("blog1", "description1", 1) { Id = 1, SoftDeletedAt = DateTimeOffset.UtcNow }
            ];

            _blogDbSetMock = _setUp.DbContextMock.SetDbSet(db => db.Blogs, blogs);

            // Act
            IActionResult result = await _blogController.RestoreAsync(1, 2, CancellationToken.None);
        ...
        [Fact]
        public async Task Restore_Conflict()
        {
            // Arrange
            List<Blog> blogs =
            [
                new("blog1", "description1", 1) { Id = 1 },
                new("blog2", "description2", 2) { Id = 2 },
                new("blog3", "description3", 1) { Id = 3, SoftDeleteLevel = 1, SoftDeletedAt = DateTimeOffset.UtcNow }
            ];

            _blogDbSetMock = _setUp.DbContextMock.SetDbSet(db => db.Blogs, blogs);

            // Act
            IActionResult result = await _blogController.RestoreAsync(3, 1, CancellationToken.None);

            // Assert
            Utils.VerifyConflictResult(result);
            _blogDbSetMock.Verify(d => d.FindAsync(new object[] { 3 }, CancellationToken.None), Times.Once);
        }
        ...
        public async Task Restore_NoContent()
        {
            ...
            List<Blog> blogs =
            [
                new("blog1", "description1", 1) { Id = 1, SoftDeleteLevel = 1, SoftDeletedAt = DateTimeOffset.UtcNow }
            ];
